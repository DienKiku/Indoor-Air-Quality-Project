...
| **Week 6** | PCB Assembly & Web App UI | **In Progress** | 80% |
| **Weeks 7 – 8** | Firmware, Firebase & Acoustic Alerting | **Pending** | 0% |
...
- [x] Complete PCB layout design based on verified schematic.
- [x] Fabricate raw PCB (board trắng), drill mounting holes, and inspect copper traces.
- [x] Perform continuity/short-circuit checks on power rails ($V_{CC}$, $GND$, $3.3V$, $5V$) before assembly.

### Week 6: PCB Assembly & Web App UI (Current Week)
- [x] Solder power subsystem (2S BMS, AMS1117), ESP32-C3, and passive components onto the PCB[cite: 1].
- [x] Mount external sensors/display (BME680, INA219, GP2Y1010AU0F, ST7735) onto the PCB[cite: 1].
- [x] Develop responsive web interface using HTML5, CSS3, and JavaScript[cite: 1].
- [x] Implement interactive time-series telemetry charts using Chart.js[cite: 1].
- [ ] Flash hardware bring-up firmware to validate populated PCB connections. *(Pending)*
